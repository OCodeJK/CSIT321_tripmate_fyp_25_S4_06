...
export default function TripPlanner() {
  ...
  const handleSubmit = async (e) => {
    ...
    // --------- Changed - Before, only calculates one flight, then drives through remaining destinations ------------------------------- \\
    if (isInternational) {
      ...
      // Build a complete route: origin + destinations
      const fullRoute = [origin, ...cleaned];

      for (let i = 0; i < fullRoute.length - 1; i++) {
        const start = fullRoute[i];
        const end = fullRoute[i + 1];

        // Check if this segment crosses countries
        if (start.country !== end.country) {
          // INTERNATIONAL SEGMENT - Need to fly
          ...
          if (!startAirport || !endAirport) {
            alert(`Could not find airports for segment ${i + 1}`);
            continue;
          }

          // Drive to departure airport
          await new Promise((resolve) => {
            directionsService.route(
              {
                origin: { lat: start.lat, lng: start.lng },
                destination: { lat: startAirport.lat, lng: startAirport.lng },
          ...
          // Add flight path
          allFlightPaths.push({
            originAirport: startAirport,
            destAirport: endAirport,
          });

          allAirports.push(startAirport, endAirport);
          ...
          totalDist += flightDist;

          // Drive from arrival airport to destination
          await new Promise((resolve) => {
            directionsService.route(
              {
                origin: { lat: endAirport.lat, lng: endAirport.lng },
            ...
            directionsService.route(
              ...
              (result, status) => {
                if (status === "OK" && result) {
                  ...
                  if (route && route.legs) {
                    const dist = route.legs.reduce((sum, leg) => sum + (leg.distance?.value || 0), 0) / 1000;
                    totalDist += dist;
                  }
                }
                resolve();
      ...
      setFlightPaths(allFlightPaths);
      setAirportMarkers(allAirports);
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <p>
        ...
        {routeMode === "INTERNATIONAL" && `✈️ Total Distance (including flights): ~${totalDistance} km`}
      </p>
      ...
      >
        {directions.map((dir, idx) => (
          <DirectionsRenderer key={idx} directions={dir} options={{ preserveViewport: true }} />
        ))}
        ...
        {flightPaths.map((flight, idx) => (
          <Polyline
            key={`flight-${idx}`}
            path={[
              { lat: flight.originAirport.lat, lng: flight.originAirport.lng },
              { lat: flight.destAirport.lat, lng: flight.destAirport.lng },
            ]}
            options={{
              strokeColor: "#1E90FF",
              strokeOpacity: 0.8,
